...
export const EditFleetItem: React.FC<EditFleetItemProps> = ({ itemId, onBack, onSuccess }) => {
  ...
  // Initialize state with the item data
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50">
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Assigned Driver/Operator</label>
                    <input className="w-full p-2 border rounded-lg" value={formData.assignedTo || ''} onChange={e => handleChange('assignedTo', e.target.value)} />
                </div>
                 <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Current Meter ({formData.type === 'VEHICLE' ? 'KM' : 'Hrs'})</label>
        ...
        {/* Technical Details */}
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-4">
            <h3 className="font-bold text-slate-700 border-b pb-2">Technical Details</h3>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">VIN / Chassis No.</label>
                ...
                <div>
                    ...
                    <input className="w-full p-2 border rounded-lg" value={formData.technicalDetails?.engineType || ''} onChange={e => handleTechChange('engineType', e.target.value)} />
                </div>
                 <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Fuel Type</label>
                    <input className="w-full p-2 border rounded-lg" value={formData.technicalDetails?.fuelType || ''} onChange={e => handleTechChange('fuelType', e.target.value)} />
                </div>
                <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1">Tire Size</label>
                    <input className="w-full p-2 border rounded-lg" value={formData.technicalDetails?.tireSize || ''} onChange={e => handleTechChange('tireSize', e.target.value)} />
                </div>
            </div>
        </div>
